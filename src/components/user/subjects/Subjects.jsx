...
function Subjects(props) {
      ...
      const newArr = await Promise.all(
        Object.keys(props.subjects[props.branch]).map(async (subject, subjectIndex) => {
          ...
          if (isTrue) {
            index += 1;
            let curIndex = index;
            return <SubjectItem key={subjectIndex} userInformation={props.userInformation} branch={props.branch} cardsNumber={cardsNumber} firstRender={firstRender} colorIndex={curIndex - 2} index={curIndex} subject={props.subjects[props.branch][subject]} deleteSubject={deleteSubject} showEdit={showEdit} setAddNew={setAddNew} setCurrentSubject={props.setCurrentSubject} />;
          }
        })
      );
      setItems([...newArr]);
  ...
  async function deleteSubject(id) {
    const response = await fetch(`${import.meta.env.VITE_URL}/admin-training/subject/delete/${id}`, { ...requestOptions, method: "delete", headers: { ...requestOptions.headers, authorization: props.userInformation.token } });
    ...
    <>
      ...
      <div className="branches">
        ...
        <div className="wrapper">
          {checkPermissions(props.userInformation, ["admin_training.subject.add", "admin_training.subject.all"], [], props.branch) ? (
            <motion.li
              className="add-box"
