...
function AddCourseForm(props) {
  ...
  const [firstStep, setFirstStep] = useState({
    ...
    subjectId: "",
    hours: 0,
    leastHoursForCertificate: 0,
    leastStudentForStart: 0,
    maxCountStudent: 0,
    price: 0,
    lessonsNumber: 0,
    note: "",
  ...
  useEffect(() => {
    if (currentClass) {
      const populateArray = async () => {
        let classCoursesIds = [];
        // Object.keys(props.currentSchedule[currentClass]).map(async (dateKey, dateIndex) => {
        //   if (dateKey < thirdStep.endDate && dateKey > thirdStep.startDate) {
        //     await Promise.all(
        //       props.currentSchedule[dateKey][currentClass].map(async (session) => {
        //         if (session.courseId != -1) classCoursesIds = [...classCoursesIds, session.courseId];
        //       })
        //     );
        //   }
        // });
  ...
  async function send() {
    ...
    setDuringAdd(true);
    const response = await fetch("http://localhost:3001/admin-training/courses/add", infoRequestOptions);
    ...
    if (data.success) {
      ...
      props.toast.success("تمت إضافة الدورة", {
        ...
        <>
          ...
          <form>
            <div className="row">
              ...
              <SelectInput
                label={"المادة :"}
                addNew={checkPermissions(props.userInformation, ["admin_training.subject.add", "admin_training.subject.all"], [], props.branch) ? props.setAddSubject : false}
                placeholder={"اختر المادة..."}
                list={props.subjects[props.branch]}
                name={"subjectId"}
                onChange={handleSave}
                state={firstStep}
                setState={setFirstStep}
                errors={firstStepErrors}
                setErrors={setFirstStepError}
