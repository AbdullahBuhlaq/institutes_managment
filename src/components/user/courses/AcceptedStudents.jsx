...
function AcceptedStudents(props) {
  ...
  async function edit() {
    ...
    if (isValid) {
      ...
      setDuringAdd(true);
      const response = await fetch(`http://localhost:3001/admin-training/courses/changeState/${id}`, infoRequestOptions);
  ...
  async function deleteStudents() {
    ...
    if (isValid) {
      ...
      setDuringAdd(true);
      const response = await fetch(`http://localhost:3001/admin-training/courses/deleteStudent/${id}`, infoRequestOptions);
      const data = await response.json();
      // const data = { success: true, data: 4 };
      if (data.success) {
        console.log(data);
  ...
  useEffect(() => {
    ...
    const populateArray = async () => {
      const newArr = await Promise.all(
        currentStudents.map(async (pendingStudent, pendingStudentIndex) => {
          ...
          //   if (isTrue) {
          if (props.course.students[pendingStudentIndex].state == "مقبول") {
            index += 1;
            return (
              <AcceptedStudentItem
    ...
    populateArray();
  }, [currentStudents, cardsNumber, studentsToEdit, studentsToDelete]);
        ...
        <form>
          <div className="button-container" style={{ justifyContent: "end" }}>
            ...
            <button
              ...
              disabled={!studentsToDelete.length || duringAdd}
            >
              حذف
            </button>
          </div>
